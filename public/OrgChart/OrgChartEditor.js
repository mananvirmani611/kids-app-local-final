...
var OrgChartNode = function (parent, boss) {
	...
	this.getCell(1, 1).setFont(
		new Font("Rubik", 3.5, true /*bold*/, false /*italic*/));
	this.getCell(1, 2).setFont(
		new Font("Rubik", 3.5, true /*bold*/, false /*italic*/));
	this.getCell(1, 0).setFont(
		new Font("Rubik", 3.5, true /*bold*/, false /*italic*/));
	this.getCell(1, 3).setFont(
		new Font("Rubik", 3, false /*bold*/, false /*italic*/));
	this.configureCells();
...
$(document).ready(function () {
	...
	diagram.addEventListener(Event.clicked, function (diagram, eventArgs) {
		...
		if (button === 2) {
			...
			node.setBounds(new Rect(position.x, position.y, 40, 40));
			node.resize();
	...
	ceoNode.setBounds(allUserData[0].bounds);
	ceoNode.setTitle(allUserData[0].title);
	ceoNode.setFullName("John Smith");
	ceoNode.setImageLocation("./OrgChart/ceo.png");
	...
	for (let i = 1; i < allUserData.length; i++) {
		if (Object.hasOwnProperty.call(allUserData, i)) {
			const element = allUserData[i];
			let parentNode = diagram.items.filter(v => v.id == element.bossId)

			 let {x,y,width,height} = element.bounds
			var myNode = new OrgChartNode(diagram, parentNode[0]);
			myNode.setBounds(new Rect(x,y,width,height));
			myNode.setTitle(element.title);
			myNode.setFullName(element.fullName);
			myNode.setImageLocation(element.imgUrl);
			myNode.setComment(element.comments);
			myNode.resize();
			diagram.addItem(myNode);
			totalNode.push({parentNode:parentNode[0],myNode});

		}
	}
	for (let j = 0; j < totalNode.length; j++) {
		const item = totalNode[j];
		var link2 = diagram.getFactory().createDiagramLink(item.parentNode, item.myNode);
		styleLink(link2);
	}
